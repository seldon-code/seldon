...
#include <config_parser.hpp>#include "directed_network.hpp"
#include <filesystem>
...
TEST_CASE( "Test reading in the network from a file", "[io_network]" )
{
    ...
    using DirectedNetwork = DirectedNetwork<AgentT>;

    auto proj_root_path = fs::current_path();
    auto network_file   = proj_root_path / fs::path( "test/res/network.txt" );

    auto network = Seldon::DirectedNetworkGeneration::generate_from_file<AgentT>( network_file );

    REQUIRE( network.n_agents() == 3 );

    std::vector<std::vector<int>> neighbours_expected                   = { { 2, 1 }, {}, { 1 } };
    std::vector<std::vector<DirectedNetwork::WeightT>> weights_expected = { { 0.1, -0.2 }, {}, { 1.2 } };
